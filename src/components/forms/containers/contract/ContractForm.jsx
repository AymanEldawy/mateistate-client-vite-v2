import {
  ...
  mergePatternWithContractData,
  onWatchChangesInTab1,
} from "@/helpers/contract/contractHelpers";
...
const ContractForm = ({ tab, code, pattern, refetch }) => {
    ...
    const subscription = watch((value, { name, type }) => {
      switch (name) {
        case "contract.endDurationDate":
          ...
          if (
            type === "change" &&
            watch("contract.startDurationDate") &&
            watch("contract.endDurationDate") &&
            Date.parse(watch("contract.endDurationDate")) <
      ...
      if (name?.indexOf("contract.") !== -1) {
        onWatchChangesInTab1(name?.split(".")?.at(-1), setValue, watch);
      }
    });
  ...
  const displayForm = () => {
    switch (tab) {
      case GLOBAL_CONTRACT_STEPS.payments:
        return <ContractFormPayments refetch={refetch} />;
